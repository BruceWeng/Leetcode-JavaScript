 ...
 * 2. For 2nd tallest group (and the rest), insert each one of them into (S) by 
 *    k value. So on and so forth.
 * 
 * 1. people.sort((a, b) => {
 *      if a[0] === b[0]: a[1] - b[1] else: b[0] - a[0]
 * });
 * 2. let result = []
 * 3. for (let p of people) result.splice(p[1], p)
 * 4. return result
 * 
 * T: O(nlogn), S: O(n)
 */
...
const reconstructQueue = function(people) {
  ...
  for (let p of people) result.splice(p[1], 0, p);
